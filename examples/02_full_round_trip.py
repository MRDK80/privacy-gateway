...
Детекция и блокирующие правила выполняются внутри ``prepare``: при
срабатывании fail-closed защищённый текст не возвращается, поднимается
``DetectionError``. Валидация ответа обработчика выполняется внутри
``restore`` в строгом режиме: неизвестный или искажённый токен приводит к
``StrictTokenError``, нарушение целостности артефактов — к
``IntegrityError``. Сценарии блокировки секрета и недопустимых токенов
ведутся отдельными задачами epic и в этот пример не входят.
...
SYNTHETIC_LETTER = (
    ...
    f"По договору № {UNSUPPORTED_DOCUMENT} работы по «{SYNTHETIC_PROJECT}»\n"
    f"приняты {SYNTHETIC_DATE} на сумму {SYNTHETIC_AMOUNT}\n"
