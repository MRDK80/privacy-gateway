...
к `requirement` и `required_fix`. Поле отбрасывается целиком, если после
фильтрации по алфавиту сводок gate в нём не остаётся текста, обнаружен
фрагмент, похожий на ключевой материал, либо остаётся признак пути.

`redaction.version` равен `2`; форма версии `1` читается без ошибок.
