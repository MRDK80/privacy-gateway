...
## Поправка 2026-09-15: профиль repository-diff-only отложен

Исходная редакция этого ADR допускала сокращённый профиль
`repository-diff-only` версии 1 для промежуточных repair-итераций. Это
решение отменяется до его реализации.

**Причина.** Сокращённый профиль не запускает `pytest`, `ruff` и `mypy`,
поэтому repair-итерация, исправляющая упавшую проверку качества, не может
быть подтверждена таким профилем. Двухуровневое гейтирование вводит второй
класс доказательств и противоречит требованию полноты evidence, из-за
которого создан этот ADR.

**Решение.** Production-путь оркестратора всегда использует профиль
`repository-full` версии 1, включая каждую repair-итерацию. Профиль
`repository-diff-only` не реализуется, отсутствует в перечислении допустимых
значений `profile` и не имеет ни одной точки вызова. Единственное условие
допуска к контроллеру: `profile` равен `repository-full`, `profile_version`
равен 1, `complete` равно `true`, `status` равен `passed`, а множества
`expected_checks` и `executed_checks` совпадают.

**Инвариантность по итерациям.** Значения `profile`, `profile_version` и
`expected_checks` не зависят от номера repair-итерации. Условие прохода
не содержит ветвлений по `repair_iteration`.

**Отложенная альтернатива.** Введение сокращённого профиля возможно только
отдельным ADR и отдельной задачей, на основании фактических длительностей
из `RetrospectiveStore` (`duration_seconds`, `repair_loops`, `iterations`).
Даже после введения такой профиль не даёт права вызвать контроллер:
последнее evidence перед review обязано быть `repository-full`.

**Совместимость.** `_default_gate()` не удаляется и остаётся достижимым
через параметр `gate` функции `run()`, но не является реализацией
