...
Порядок проверок (обязателен, ADR-16):
...
Классификация токенов (ADR-17):
    - Известный       — подставить значение во все вхождения (ADR-19).
    ...
    - Пропавший       — всегда предупреждение, не ошибка (ADR-18).
    - Дублированный   — подстановка во все вхождения, счётчик в отчёт (ADR-19).

Отчёт (RestoreResult) содержит только счётчики и токены — без значений (ADR-16).
...
ADR-17: Классификация токенов (пять категорий).
ADR-18: Пропавший токен — предупреждение, не ошибка.
ADR-19: Дублированный токен — подстановка во все вхождения.
ADR-20: Чувствительность к регистру — токены регистрозависимы ([EMAIL_1] ≠ [email_1]).
"""
...
# N — целое число >= 1 (ADR-20: регистрозависимо).
...
def _classify_candidates(
    ...
    Регистрозависимо (ADR-20): [email_1] — искажённый, [EMAIL_1] — известный.
...
def _substitute(
    ...
    """Подставить значения для всех известных токенов (ADR-19).
...
def restore_text(
    ...
    Порядок проверок строго соблюдается (ADR-16, раздел 5.1 хендовера):
    ...
    # Дублированные токены — более одного вхождения (ADR-19)
    for token, positions in known.items():
        if len(positions) > 1:
    ...
    # Предупреждения о пропавших токенах (ADR-18 — не ошибка, но обязательно в отчёт)
