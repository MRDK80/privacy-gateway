...
- Удаление рабочего каталога в `discard` выполняется над проверенной сущностью
  ...
  (#43, дополнение ADR-34). Подмена имени или объекта после последней проверки
  не приводит к удалению постороннего каталога. На платформах с
  descriptor-relative семантикой каталог дополнительно закрепляется открытым
  дескриптором, и обход дерева идёт относительно него.
