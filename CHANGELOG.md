...
- prepare: при `overwrite=False` публикация артефактов больше не заменяет
  ...
  (#64, ADR-64).
- CI: wheel и sdist собираются в каждой cell exact CI matrix, версия
  дистрибутива сверяется в чистом окружении, нижняя граница backend поднята
  до `setuptools>=70.1` (#85).
...
- v0.3.0 и ADR-22 заявляли CLI-команду `pgw key delete`; она не входила в
  ...
  `create`, `status`, `rotate`. Библиотечная `keystore.delete_key()` остаётся
  функцией библиотеки и не является CLI-командой (#47, #42, ADR-47).
- ADR-22 обещал код 1 при повторном `pgw key create`; shipped-поведение —
  код 3. Контракт кода 3 сохранён, формулировка ADR исправлена датированным
  дополнением (#47, ADR-47).
- Строгий отказ по токенам `pgw restore` использует выделенный код 5 (ADR-21);
  формулировка `docs/SECURITY.md`, обещавшая код 3, исправлена (#47, ADR-47).
- Исторические разделы CHANGELOG и release notes v0.3.0 не переписаны:
  ошибочные обещания сохранены и снабжены датированной erratum.
...
- Проверка владения не обращается к хранилищу ключей: освобождение ресурсов
  остаётся возможным после `pgw key rotate` и вызова
  `keystore.delete_key()`.
...
[Unreleased]: https://github.com/MRDK80/privacy-gateway/compare/v0.5.0...HEAD
