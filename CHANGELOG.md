...
- Удаление рабочего каталога в `discard` выполняется над проверенной сущностью
  файловой системы, а не над заново разрешённым путём: каталог отцепляется от
  исходного имени и удаляется после сверки идентичности отцепленного объекта
  (#43, дополнение ADR-34).
- Рекурсивное удаление привязано к подтверждённой сущности на всём протяжении
  операции: обход дерева идёт от удерживаемого дескриптора (Linux) или handle
  (Windows), карантинное имя в destructive-вызовах не участвует, а при
  невозможности подтвердить закрепление не удаляется ничего и допускается
  остаточный каталог с префиксом `.pgw-quarantine-` (#52, дополнение ADR-34).
  Ранее подмена карантинного имени между последней проверкой и началом обхода
  приводила к удалению подставленного каталога.
